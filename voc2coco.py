...
def get_image_info(annotation_root, img_path,temp_img_id ,extract_num_from_imgid=True):
    path = annotation_root.findtext('path')
    if path is None:
        filename = annotation_root.findtext('filename')
    else:
        filename = os.path.basename(path)
    #print(str(img_path.split('/')[-2])+str(img_path.split('/')[-1]))
    filename = str(img_path.split('/')[-2])+'/'+str(img_path.split('/')[-1])
    #annotation_root.findtext('filename')
    #img_name = os.path.basename(filename)
    img_id = temp_img_id
    #if extract_num_from_imgid and isinstance(img_id, str):
    #    img_id = int(re.findall(r'\d+', img_id)[0])
    #temp = img_id.split('_')
    #print(annotation_root.tag)
    #print(annotation_root.attrib)
...
def get_coco_annotation_from_obj(obj, label2id):
    ...
    assert xmax > xmin or ymax > ymin, f"Box size error !: (xmin, ymin, xmax, ymax): {xmin, ymin, xmax, ymax}"
    o_width = xmax - xmin
    o_height = ymax - ymin
    ann = {
...
def convert_xmls_to_cocojson(annotation_paths: List[str],
    ...
    #paths = glob.glob('./cmes_new/images/**/*.png')
    ...
    for i, a_path in tqdm(enumerate(annotation_paths)):
        ...
        #print(ann_root)
        img_info = get_image_info(annotation_root=ann_root,img_path = i_path, temp_img_id=i+1,
